...
if (process.platform === 'linux') {
    ...
    if (fs.existsSync(xdgDir) && !fs.existsSync(path.resolve(xdgDir, 'teamost-desktop-dev.desktop'))) {
        fs.writeFileSync(
            path.resolve(xdgDir, 'teamost-desktop-dev.desktop'),
            `[Desktop Entry]
Name=Mattermost.Dev
Exec=${path.resolve(process.cwd(), 'node_modules/electron/dist/electron')} ${path.resolve(process.cwd(), 'dist')} %U
Terminal=false
Type=Application
Icon=teamost-desktop
...
`,
        ...
        fs.appendFileSync(defaultsListPath, 'x-scheme-handler/mattermost-dev=teamost-desktop-dev.desktop\n');
        ...
        fs.appendFileSync(mimeCachePath, 'x-scheme-handler/mattermost-dev=teamost-desktop-dev.desktop\n');
